    ...
    <div class="container">
        <div class="row">
            <div class="col-12 text-center">
                <h4 class="fw-bold">Search Filter App</h4>
            </div>
        </div>
        <div class="row">
            <!-- Search Filter -->
            ...
            @isset($properties)
                @foreach ($properties as $property)
                    <div class="row style-row">
                        <div class="col-6">
                            <span class="fw-bold">Property Name:</span> {{ $property->property_name }}
                        </div>
                        <div class="col-6">
                            <span class="fw-bold">Location:</span> {{ $property->location[0]->location_name }}
                        </div>
                        <div class="col-6">
                            <span class="fw-bold">Sleeps:</span> {{ $property->sleeps }}
                        </div>
                        <div class="col-6">
                            <span class="fw-bold">Beds:</span> {{ $property->beds }}
                        </div>
                        <div class="col-6">
                            <span class="fw-bold">Pets Allowed:</span> {{ $property->accepts_pets === 1 ? 'Yes' : 'No' }}
                        </div>
                        <div class="col-6">
                            <span class="fw-bold">Near Beach:</span> {{ $property->near_beach === 1 ? 'Yes' : 'No' }}
                        </div>
                        @isset($property->booking)
                            @if(count($property->booking) > 0)
                                <div class="col-12">
                                    <span class="fw-bold">Bookings:</span>
                                    <ul>
                                        @foreach($property->booking as $book)
                                           <li>ID: {{ $book->__pk }} || Start Date: {{ $book->start_date }} || End Date: {{ $book->end_date }}</li>
                                        @endforeach
                                    </ul>
                                </div>
                            @endif
